    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
      ...
      {files.map((file, index) => (
        <div key={`${file.url || file.name}-${index}`} style={{ display: "flex", alignItems: "center", gap: 6, background: "#eff6ff", border: "1px solid #bfdbfe", borderRadius: 5, padding: "4px 7px" }}>
          <a {...documentLinkProps(file)} style={{ minWidth: 0, flex: 1, color: ACCENT, fontSize: 10, textDecoration: "none", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }} title={file.name}>
            {file.name || "Document"}
          </a>
          {!readOnly && (
