            ...
            <tbody>
              {rows(faculty.externalProjects).map((r, i) => (
                <tr key={i} style={i % 2 ? { background: "#f8fafc" } : {}}>
                  ...
                  <td style={TD}><RO val={r.status} /></td>
                  <td style={TDS}><RO val={r.score} center /></td>
                  <td style={TDS_DIR}><DirInput val={getDir("externalProjects", i, "dir")} onChange={v => setDir("externalProjects", i, "dir", v)} /></td>
                </tr>
              ))}
            </tbody>
          </table>
...
function ReviewPanel({ faculty, onBack, onSubmit, readOnly = false }) {
  ...
  const calcDirScore = () => {
    ...
    const externalResProjects = (faculty.externalProjects || []).reduce((a, _, i) => a + getD("externalProjects", i, "dir"), 0);
    ...
    const prod = (faculty.products || []).reduce((a, _, i) => a + getD("products", i, "dir"), 0);
          ...
          <table style={{ ...T, marginBottom: 18 }}>
            ...
            <tbody>
              ...
              <tr style={{ background: g.bg }}>
                <td style={TD} colSpan={3}><strong>Grade (based on Director Score)</strong></td>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(2,1fr)", gap: 14 }}>
              {filtered.map(item => {
                ...
                const partA = [
                  ...
                  ...(item.feedback || []).map(r => feedbackRowScore(r, 10)),
